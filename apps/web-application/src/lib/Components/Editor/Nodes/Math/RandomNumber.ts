...
export class RandomNumber extends BaseNode {
	...
	nodeToCode(generator: BaseGenerator): string {
		const min = this.inputs[0].link == null ? this.properties.from : generator.valueToCode(this, 0);

		const max = this.inputs[1].link == null ? this.properties.to : generator.valueToCode(this, 1);
