...
export class DataManager extends IDataManager {
    ...
    async signIn(username: string, password: string): Promise<IUser> {
        if (username === "admin" && password === "admin") {
            ...
            const usr = new User({
                name: "admin",
                avatar: data ? data.avatar : "https://lastfm.freetls.fastly.net/i/u/avatar170s/635ee48d09535311aa943c497e6656e7",
                previewProjects: data.previewProjects ? data.previewProjects.map((v: JsonPreviewProject) => {
