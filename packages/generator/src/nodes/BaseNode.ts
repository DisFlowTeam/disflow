...
declare module "litegraph.js" {
	interface LGraphNode {
		onOutputAdded(output: INodeOutputSlot): unknown;
		onInputAdded(input: INodeInputSlot): unknown;
	}
}
...
export function constructImportId(statement: AllImportDeclaration) {
	...
	if (statement.type === ImportType.Object)
		importId = `[${statement.module.sort().join("|")}]`;
...
export abstract class BaseNode extends LGraphNode {
	...
	static buildReferenceName() {
		return `${BaseNode.category}/${BaseNode.title}`;
	}

	static forEngine(engine: BaseGenerator) {
		engine.nodes.set(BaseNode.buildReferenceName(), (node, gen) => {
	...
	onOutputAdded(output: INodeOutputSlot) {
		if (BaseGenerator.isExecutionPin(output.type))
			output.shape = LiteGraph.ARROW_SHAPE;
	}

	onInputAdded(input: INodeInputSlot) {
		if (BaseGenerator.isExecutionPin(input.type))
			input.shape = LiteGraph.ARROW_SHAPE;
	}
