import type { INodeInputSlot, LGraph } from "litegraph.js";
import {
	type BaseNode,
	ImportType,
	OnProgramStartNode,
...
export class JavaScriptGenerator extends BaseGenerator {
	...
	valueToCode(node: BaseNode, inputIndex: number): string {
		...
		// biome-ignore lint: .has() ensures codeCache has the specific node
	...
	statementToCode(node: BaseNode, outputIndex: number): string {
		let finalCode = "";
		const oNodes =
			(node.getOutputNodes(outputIndex) as BaseNode[] | null) || [];
		...
		// biome-ignore lint: hasGenerator ensures the generator function can be executed
	...
	walkDownStream(node: BaseNode) {
		...
		while (processingNode) {
			...
			// biome-ignore lint: hasGenerator guards the generator function
	...
	graphToCode(graph: LGraph): {
		...
		if (roots.length === 0)
			throw new GenerationError("Node roots were found in the graph.");

		const defaultRootIndex = roots.findIndex(
			(node) => node instanceof OnProgramStartNode,
		);

		if (defaultRootIndex === -1)
			throw new GenerationError(
				"No 'On Program Start' root node found in the graph.",
			);

		const defaultRoot = roots.splice(
			defaultRootIndex,
			1,
		)[0] as OnProgramStartNode;
		...
		for (const root of roots) {
			...
			// biome-ignore lint: hasGenerator function called previously
		...
		const intents = ["Guilds", ...structuredClone(this.intents)].map(
			(v) => `\tDisflowDJS.GatewayIntentBits.${v}`,
		);
			...
			meta: JSON.stringify(
				{
					...
					version: "1.0.0",
					description:
						"This Discord bot is generated using DisFlow. The open source, no-code Discord bot builder",
	...
	collectImports(node: BaseNode) {
		for (const statement of node.imports) {
			if (
				...
				!this.initialisers.has(statement.initialiser)
			)
			...
			if (statement.type === ImportType.Object) {
				...
				if (this.imports.has(modId)) {
					// biome-ignore lint: .has function called previously
